...
for(var i=0;i<this._monitoredUpdatePanels.length;i++){if(panelID.ID.match(this._monitoredUpdatePanels[i].UpdatePanelID)!==null){return true;}}
return false;},_isDisableAllElementsPanel:function(panelID){if(this._monitoredUpdatePanels===null){return false;}
if(panelID===null){return false;}
for(var i=0;i<this._monitoredUpdatePanels.length;i++){if(panelID.ID.match(this._monitoredUpdatePanels[i].UpdatePanelID)!==null){return this._monitoredUpdatePanels[i].DisableAllElements;}}},_findContainingPanel:function(el){if(el===null||this._monitoredUpdatePanels===null)
return null;if(el.id){for(var i=0;i<this._monitoredUpdatePanels.length;i++){if(el.id.match(this._monitoredUpdatePanels[i].UpdatePanelID)!==null){var foundPanel={};foundPanel.ClientID=el.id;foundPanel.ID=this._monitoredUpdatePanels[i].UpdatePanelID;return foundPanel;}}}
return this._findContainingPanel(el.parentNode);},get_waitText:function(panelID){if(this._monitoredUpdatePanels){for(var i=0;i<this._monitoredUpdatePanels.length;i++){if(panelID.ID.match(this._monitoredUpdatePanels[i].UpdatePanelID)!==null){return this._monitoredUpdatePanels[i].WaitText;}}}
if(this._waitText!==null){return this._waitText;}
return null;},get_waitImage:function(panelID){if(this._monitoredUpdatePanels){for(var i=0;i<this._monitoredUpdatePanels.length;i++){if(panelID.ID.match(this._monitoredUpdatePanels[i].UpdatePanelID)!==null){return this._monitoredUpdatePanels[i].WaitImage;}}}
if(this._waitImage!==null){return this._waitImage;}
return null;},_disableAllElements:function(Panel){var panel=document.getElementById(Panel.ClientID);if(panel!==null){var inputs=panel.getElementsByTagName('input');for(var i=0;i<inputs.length;i++){if(inputs[i].type=='submit'||inputs[i].type=='button'||inputs[i].type=='image'||inputs[i].type=='checkbox'){inputs[i].disabled=true;}}
...
else if(element.type==='checkbox'){element.disabled=true;element.blur();}
